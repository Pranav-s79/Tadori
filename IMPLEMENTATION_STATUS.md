...
Current node: 08-07 — Path/route/test/doc explore displays: IMPLEMENTED on branch bp/08-07-explore-displays (viz only), pending CI/merge. 09-04 FULLY MERGED (#35=e72503f).
Branch: bp/08-07-explore-displays (off main e72503f)
Latest commit: e72503f main (PR #35 merged — 09-04 changed_with, CI green both OSes). 09-03 merged (#33/#34). 09-02 merged (#27/#29/#30/#31/#32).
Open PR: 08-07 to be opened after push.
...
Next frontier: after 08-07 merges — the BehaviorStory frontend StoryView is now UNBLOCKED (its 08-07 explore-display dependency is delivered). The server-side rich-derived engine (pathOutputSchema parity, per-test linkage, route edge-origin, docs grouping) remains a follow-up that would upgrade these honest stub-backed displays. changed_with edges surface through GET /api/v1/edges as ordinary edges (no dedicated overlay yet).
Known blocker: none
