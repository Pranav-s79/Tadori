...
Current node: 08-07B — rich /tests linkage engine (server+viz): IMPLEMENTED on branch bp/08-07b-rich-tests, pending CI/merge. 08-07A StoryView MERGED (#37=b4ffd51). 08-07 explore MERGED (#36). 09-04 MERGED (#35).
Branch: bp/08-07b-rich-tests (off main b4ffd51)
Latest commit: b4ffd51 main (PR #37 merged — 08-07A StoryView, CI green both OSes). 08-07 (#36). 09-04 (#35). 09-03 (#33/#34). 09-02 (#27/#29/#30/#31/#32).
Open PR: 08-07B to be opened after push.
...
Next frontier: after 08-07A merges — candidates are the server rich-derived engine (pathOutputSchema parity + in-process-vs-HTTP parity test, per-test linkage kinds, route routes_to edge-origin, docs grouping) which would upgrade the 08-07 stub-backed displays; and 08-11 (browser a11y / full-flow Chromium suite, which names the path/story displays). changed_with edges surface through GET /api/v1/edges as ordinary edges (no dedicated overlay yet).
Known blocker: none
