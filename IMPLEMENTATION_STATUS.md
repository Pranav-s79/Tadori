...
Current node: 08-07E — /path full pathOutputSchema + parity (server+viz): IMPLEMENTED on branch bp/08-07e-path-parity, pending CI/merge. COMPLETES the derived-engine follow-ups. 08-07D /docs grouping MERGED (#40=63301b8). 08-07C (#39). 08-07B (#38). 08-07A (#37). 08-07 (#36). 09-04 (#35).
Branch: bp/08-07e-path-parity (off main 63301b8)
Latest commit: 63301b8 main (PR #40 merged — 08-07D /docs grouping, CI green both OSes). 08-07C (#39). 08-07B (#38). 08-07A (#37). 08-07 (#36). 09-04 (#35). 09-03 (#33/#34). 09-02.
Open PR: 08-07E to be opened after push.
...
Next frontier: after 08-07E merges — the server derived-engine follow-ups are COMPLETE (08-07B per-test linkage, 08-07C route path-source, 08-07D docs grouping, 08-07E path full-shape+parity). The explore displays now render the real derived data, not stubs. Open frontier: 08-11 (browser a11y / Chromium full-flow suite — names the path/story displays as required flow steps; would validate all the 08-07* surfaces end-to-end), and the standalone BehaviorStory viz-graph overlay (StoryView is list-form today; a canvas overlay of the story path is a possible enhancement). changed_with edges surface through GET /api/v1/edges as ordinary edges (no dedicated overlay yet).
Known blocker: none
