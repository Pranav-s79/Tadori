...
Current node: 08-07C — /routes path-source origin (server+viz): IMPLEMENTED on branch bp/08-07c-route-path-source, pending CI/merge. 08-07B rich /tests MERGED (#38=2ea8bcb). 08-07A StoryView (#37). 08-07 explore (#36). 09-04 (#35).
Branch: bp/08-07c-route-path-source (off main 2ea8bcb)
Latest commit: 2ea8bcb main (PR #38 merged — 08-07B rich /tests linkage, CI green both OSes). 08-07A (#37). 08-07 (#36). 09-04 (#35). 09-03 (#33/#34). 09-02 (#27/#29/#30/#31/#32).
Open PR: 08-07C to be opened after push.
...
Next frontier: after 08-07C merges — remaining derived-engine slices are (a) /docs grouping-by-documented-entity (docs endpoint carries no `documents` edges yet), and (b) /path full pathOutputSchema (status/multi-path/nearestApproach) + the in-process-vs-HTTP parity test the 08-07 blueprint specifies. Done so far: per-test linkage (08-07B), route path-source origin (08-07C). Also open: 08-11 (browser a11y / Chromium full-flow, names the path/story displays). changed_with edges surface through GET /api/v1/edges as ordinary edges (no dedicated overlay yet).
Known blocker: none
