...
interface PathFinderProps {
  ...
  /** The origin: the inspected entity. Only the destination is asked for. */
  from: string;
}
...
/**
 * Path display: resolve a path from the inspected entity to a named one and
 * render the full path-tool result. The result's own `status` drives distinct
 * honest UI: `ok` shows the found
 ...
 */
export function PathFinder({ onInspect, from }: PathFinderProps): ReactElement {
  ...
  async function onSubmit(event: FormEvent): Promise<void> {
    ...
    try {
      const result = await fetchPath(from, to.trim());
  ...
  return (
    <section className="explore-path" aria-label="Path finder">
      <form onSubmit={(e) => void onSubmit(e)}>
        <label>
          To
          <input type="text" value={to} autoFocus onChange={(e) => setTo(e.target.value)} placeholder="entity key or name" />
        </label>
